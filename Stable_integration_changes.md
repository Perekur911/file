...
- первые две формы с именованными диапазонами, чтобы ширина формы определялась
  как разница столбцов `Stable_2_sampleCode` и `Stable_1_sampleCode`.
...
Для каждой формы нужны имена `Stable_<номер>_<заголовок>` для всех перечисленных
полей, кроме `dateTimeSync`. Например:
...
Имена должны быть уровня книги. Диапазон `Stable_<n>_pressureAbs` является
связанной ячейкой чекбокса.
...
```vb
        ElseIf taskType = "Stable" Then
            ...
            Set cellSearch = _
                Stable_list.Range("Stable_1_TaskId") _
                           .EntireRow.Find( _
                               cellX.value, _
                               LookAt:=xlWhole _
                           )
            ...
            If cellSearch Is Nothing Then
                ...
                With Stable_list.Range( _
                    "Stable_" & emptyStablePage & "_TaskId" _
                ).MergeArea
                    .value = cellX.value
                ...
                With Stable_list.Range( _
                    "Stable_" & emptyStablePage & "_sampleCode" _
                ).MergeArea
                    .value = _
