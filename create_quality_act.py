...
def parse_number_for_excel(value: Any) -> Any:
    ...
    # Обычное число или число в экспоненциальной записи:
    # -8,76385985975E-02
    # 1.25E+03
    # 7e-5
    number_pattern = (
    ...
    if not re.fullmatch(number_pattern, compact):
        return original
...
def format_date_range(min_date: Any, max_date: Any) -> str | None:
    min_date = normalize_excel_date(min_date)
    max_date = normalize_excel_date(max_date)
    ...
    min_text = min_date.strftime("%d.%m.%Y")
    max_text = max_date.strftime("%d.%m.%Y")
...
def create_quality_act(
    *,
    ...
    lif_headers, lif_rows = read_xlwings_table(
        target_book,
        sheet_name="ProjectLIF",
        table_name="ProjectLIF",
    )
    ...
    try:
        ...
        # -------------------------------------------------------------------------
        # 1. Заполняем LIF-часть из ProjectLIF
        # -------------------------------------------------------------------------
...
def get_op_vliqphase_for_act(
    result_table: ResultTable,
    result_row_index: int,
) -> Any:

    vliq = result_table.get_value(
        result_row_index,
        "vliqphase",
    )

    # Есть настоящее числовое значение:
    # 0, 1, 1.4, 5 и т.д.
    if vliq is not None and normalize_text(vliq) != "":
        return vliq

    qualifier = normalize_text(
        result_table.get_value(
            result_row_index,
            "liqphasequalifier",
        )
    )

    if qualifier == "Менее 1 мл":
        return "Менее 1 мл"
    ...
    for row_num_act in range(1, row_count + 1):
        ...
        for result_code, result_table in results.items():
            ...
            for col_name, act_col_idx in result_fields.items():
                if (result_code == "OP" and col_name == "vliqphase"):
                    value = get_op_vliqphase_for_act(
                        result_table,
                        result_row_index,
                    )

                else:
                    value = result_table.get_value(
                        result_row_index,
                        col_name,
                    )
...
def calculate_gas_quality(
    ...
    calc_col = gas_map.get(
        "locate", {}
    ).get("calc")

    delta_col = gas_map.get(
        "OP", {}
    ).get("deltapopenpct")

    vliq_col = gas_map.get(
        "OP", {}
    ).get("vliqphase")

    if (
        calc_col is None
        or delta_col is None
        or vliq_col is None
    ):
        return
    ...
    for row in range(1, gas_cur_row + 1):
        ...
        delta_popen = to_float_or_none(
            gas_act_tbl[row][delta_col]
        )

        raw_vliq = normalize_text(
            gas_act_tbl[row][vliq_col]
        )

        if delta_popen is None:
            continue

        # Отдельная семантика "менее 1 мл":
        # раньше числа 0 < x < 1 попадали именно
        # в "Качественная с отклонением".
        if raw_vliq == "Менее 1 мл":
            if delta_popen < -10:
                gas_act_tbl[row][calc_col] = (
                    "Некачественная"
                )
            else:
                gas_act_tbl[row][calc_col] = (
                    "Качественная с отклонением "
                    "от требований методики отбора"
                )

            continue

        # Прочерк означает, что числового результата нет.
        # Сохраняем прежнюю логику: считаем как 0.
        if raw_vliq in {"", "-"}:
            v_liq_ph = 0.0
        else:
            v_liq_ph = to_float_or_none(
                raw_vliq
            )
...
def calculate_liquid_quality(
    ...
    for row in range(1, liq_cur_row + 1):
        ...
        if delta_popen < -10 or pct_gas_ph >= 5.0:
            ...
            liq_act_tbl[row][calc_col] = "Качественная с отклонением от требований методики отбора"
