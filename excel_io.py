...
class ExcelWorkbookGateway:
    ...
    def replace_table_rows(self, schema: TableSchema, db_rows: list[dict[str, Any]]) -> int:
        """
        Полностью заменяет данные умной таблицы Excel данными из БД.

        Используется для синхронизации БД -> Excel.
        Заголовки таблицы не трогаются. Данные записываются одним прямоугольным блоком.
        Порядок колонок берётся из текущего порядка заголовков Excel.
        Лишние строки старой таблицы удаляются через Resize ListObject.
        """
        table = self._get_list_object(schema)
        headers = self._read_headers(table)
        self._validate_headers(schema, table.Name, headers)
        ...
        # В Excel ListObject удобнее держать минимум одну data-строку.
        # Если данных нет, оставляем одну пустую строку под заголовком.
        target_data_rows = max(row_count, 1)
        self._resize_list_object(table, data_rows=target_data_rows, col_count=col_count)

        body = table.DataBodyRange
        if body is None:
            return row_count

        excel_rows = ValueConverter.rows_db_to_excel(schema, db_rows)
        matrix = self._build_excel_matrix(schema, headers, excel_rows)
        if row_count == 0:
            matrix = [[None for _ in range(col_count)]]
